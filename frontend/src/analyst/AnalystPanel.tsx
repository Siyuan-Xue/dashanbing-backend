...
function AnalystBody({ source, accountId, onEvidence, style, onStyle, ready = true }: Props & { style: AnalystStyle; onStyle: (value: AnalystStyle) => void }) {
  ...
  useEffect(() => {
    ...
    analystApi.context(source, controller.signal).then(value => { if (!controller.signal.aborted) setContext(value); }).catch(() => { if (!controller.signal.aborted) { setContext(null); setContextError(true); } });
