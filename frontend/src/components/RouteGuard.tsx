...
export function RouteGuard() {
  ...
  if (authError) {
    return <main className="route-state"><section className="route-error" role="alert"><h1>{t("authCheckFailed")}</h1><p>{t("authCheckFailedBody")}</p><button className="button button-primary" type="button" onClick={() => void refresh()}>{t("retry")}</button></section></main>;
  }
  if (!user) {
