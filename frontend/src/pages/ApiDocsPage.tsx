...
TASK_ID=$(curl -fsS -X POST "$BASE_URL/api/v1/tasks" \\
  ...
  -d '{"title":"Friday shooting","mode":"quick"}' | python -c 'import json,sys; print(json.load(sys.stdin)["id"])')
...
curl -fsS "$BASE_URL/api/v1/tasks/$TASK_ID" -H "Authorization: Bearer $API_KEY"
curl -fsS "$BASE_URL/api/v1/tasks/$TASK_ID/result" -H "Authorization: Bearer $API_KEY"`;
