...
export function SettingsPage() {
  ...
  return <div className="workspace-page settings-page">
    ...
    {error ? <WorkspaceState title={wt("loadFailed")} body={error.message} onRetry={reload}/> : !usage ? <div className="loading-block" role="status"/> : <>
      ...
      <section className="settings-section"><div className="settings-heading"><span><Icon name="clock"/></span><div><h2>{wt("retention")}</h2><p>UTC</p></div></div><dl className="retention-grid"><div><dt>{wt("drafts")}</dt><dd>{formatRetentionDuration(locale, usage.retention.drafts)}</dd></div><div><dt>{wt("enrollment")}</dt><dd>{formatRetentionDuration(locale, usage.retention.enrollment_data)}</dd></div><div><dt>{wt("rawInputs")}</dt><dd>{formatRetentionDuration(locale, usage.retention.raw_inputs)}</dd></div><div><dt>{wt("results")}</dt><dd>{formatRetentionDuration(locale, usage.retention.results)}</dd></div></dl></section>
    </>}
