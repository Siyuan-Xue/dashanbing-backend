...
export function TaskListPage() {
  ...
  const hasFilters = Boolean(requestQuery.q || requestQuery.status || requestQuery.mode);
  const clearFilters = () => {
    setSearch(""); setStatus(""); setMode(""); setFiltersOpen(false);
  ...
  return <div className="workspace-page task-list-page">
    ...
    {error ? <WorkspaceState title={wt("loadFailed")} body={error.message} onRetry={() => setRevision((value) => value + 1)}/> : loading ? <div className="loading-block" role="status" aria-label={wt("detailLoading")}/> : tasks.length === 0 ? <WorkspaceState icon={hasFilters ? "search" : "file"}
      title={hasFilters ? wt("emptyTasks") : locale === "zh" ? "还没有任务" : "No tasks yet"}
      body={hasFilters ? (locale === "zh" ? "试试其他关键词，或清除筛选查看所有任务。" : "Try another search or clear filters to see all tasks.") : (locale === "zh" ? "创建第一个任务，开始回顾你的篮球训练。" : "Create your first task to review your basketball training.")}
      action={hasFilters ? <button className="button button-outline" type="button" onClick={clearFilters}>{locale === "zh" ? "清除筛选" : "Clear filters"}</button> : page > 1 ? <button className="button button-outline" type="button" onClick={() => changePage(1)}>{locale === "zh" ? "返回第一页" : "Go to first page"}</button> : <Link className="button button-primary" to="/workspace/new"><Icon name="plus"/>{wt("createTask")}</Link>}/> : <div className="task-table-wrap" role="region" aria-label={wt("tasks")} tabIndex={0}><table className="task-table"><thead><tr><th>{wt("taskTitle")}</th><th>{wt("status")}</th><th>{wt("mode")}</th><th>{wt("progress")}</th><th>{wt("created")}</th><th>{wt("actions")}</th></tr></thead><tbody>{tasks.map((item) => <tr key={item.id}><td data-label={wt("taskTitle")}><Link className="task-title-link" to={`/workspace/tasks/${item.id}`}><span className="task-file-icon"><Icon name="file" size={18}/></span><span><b>{item.title}</b><small>{item.id.slice(0, 8)}</small></span></Link></td><td data-label={wt("status")}><StatusChip status={item.status} stageMessage={item.stage_message}/></td><td data-label={wt("mode")}><span className="mode-label">{taskModeLabel(locale, item.mode)}</span></td><td data-label={wt("progress")}>{["draft", "uploading"].includes(item.status) ? <span title={wt("uploadCount")}>{item.inputs.filter(input => input.validation_state === "valid").length} / 5</span> : <div className="task-progress" role="progressbar" aria-label={`${wt("progress")} · ${item.title}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={item.progress}><div className="mini-progress"><i style={{ width: `${item.progress}%` }}/></div><span>{item.progress}%</span></div>}</td><td data-label={wt("created")}><time dateTime={item.created_at}>{new Intl.DateTimeFormat(locale === "zh" ? "zh-CN" : "en", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" }).format(new Date(item.created_at))}</time></td><td data-label={wt("actions")}><div className="table-actions"><Link className="table-action" to={`/workspace/tasks/${item.id}`} aria-label={wt(["draft", "uploading"].includes(item.status) ? "continueDraft" : "open")} title={`${wt(["draft", "uploading"].includes(item.status) ? "continueDraft" : "open")} · ${item.title}`}><Icon name={["draft", "uploading"].includes(item.status) ? "upload" : "arrow"}/></Link>{["draft", "uploading", "queued", "running"].includes(item.status) && actionButton(item, "cancel")}{["failed", "canceled"].includes(item.status) && actionButton(item, "retry")}{["draft", "failed", "canceled", "completed", "expired"].includes(item.status) && actionButton(item, "delete")}</div></td></tr>)}</tbody></table></div>}
