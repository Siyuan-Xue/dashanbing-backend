...
| 设置 | 直接展示四项用量配额、保留规则、持久化语言和主题设置，无页内导航 |
...
筛选采用带漏斗图标的入口、360px 自适应弹层、浅灰选择框、自定义下拉箭头和底部重置/确认按钮，保留现有状态与分析模式字段。重置仅修改待确认值，确认后保留搜索词及每页条数并返回第一页；Escape 或点击外部会放弃未确认的更改，筛选状态标记反映实际已应用条件。短窗口允许弹层内部滚动。

设置页移除“用量与配额 / 数据保留 / 界面偏好”页内导航与占位列，三组内容直接使用主内容区。保留原有配额、保留周期和语言/主题设置。新增 320px、390px 浅深主题筛选截图，基线共 312 张，另有 70 张边界截图。

本轮验证：79 项单元测试通过，类型检查与生产构建通过，完整功能及视觉回归 361 passed、27 skipped，代码审查问题已修复并复核


### 详情顶栏与结果自动播放

任务与示例详情顶栏在桌面同一行排列，返回入口使用图标，长标题省略并提供完整标题悬停提示。任务仅保留标题、模式和状态，移除来源、短 ID 和已完成进度的重复信息；排队/运行状态保留紧凑进度条，阶段说明通过悬停提示、无障碍文本及结果区继续提供。示例页移除描述与模式说明标签，保留有无障碍名称的模式选择和“创建任务”按钮。窄屏自然分行。

结果视频默认静音、内联自动播放，保留原生播放、暂停和声音控件，切换机位后自动播放；切换概览/时间线/JSON 不会重启用户暂停的视频。测试使用无个人数据的六秒 VP8 视频验证实际播放时间推进，并覆盖任务和示例预览。浏览器若拒绝自动播放，仍可使用原生播放按钮。

本轮验证：79 项单元测试通过，类型检查与构建通过，完整功能及视觉回归 365 passed、27 skipped，独立代码审查无重大问题


### 账户菜单与退出登录

官网与 API 页的登录头像改为账户下拉菜单，显示用户名、邮箱及退出登录入口，不再跳转设置页。菜单采用紧凑浮层，适配中英文与浅深主题；点击外部、Tab 离开、Escape 和路由切换均可关闭。手机先展开导航再打开账户菜单，Escape 先关闭账户菜单并将焦点还给头像。长用户信息自然换行，矮窗口内菜单可滚动。

退出调用现有带 Cookie 的 POST /api/v1/logout 接口，提交期间禁用重复操作；成功后清除登录状态并返回首页，访问工作台重新要求登录；失败保留当前账户并允许重试。新增 8 张账户菜单截图，截图查看器支持切换语言、主题和桌面/手机宽度。

本轮验证：81 项单元测试通过，类型检查与生产构建通过，完整功能和视觉回归 379 passed、27 skipped；审查发现的退出后焦点问题修复后，公共页定向回归 19 passed、1 skipped。账户菜单新增 8 张截图，全部基线共 320 张。


### 设置退出与工作台导航选中状态

设置页右上角新增退出登录按钮，复用现有 Cookie 退出接口和双语提示。退出失败保留设置页面及按钮焦点，配额加载失败也不影响退出。成功导航和清除会话统一在认证提供器更新，避免受保护路由先跳回登录页。首页头像菜单继续使用同一退出流程。

创建任务、任务列表的当前入口采用 2px 棕红边框、棕红文字、浅棕红背景和 600 字重；未选中按钮预留相同边框宽度，切换时尺寸稳定。浅深主题、手机抽屉与悬停侧栏同步使用这些样式。

验证：81 项单元测试、类型检查和生产构建通过，全套功能和视觉回归 383 passed、27 skipped，更新受影响的工作台截图与边界截图。
