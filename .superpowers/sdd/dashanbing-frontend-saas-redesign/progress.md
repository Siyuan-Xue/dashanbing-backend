...
Task 6: fix round 2/5 (4 open — focus restoration after create-limit/revoke DOM changes, public visual auth readiness, complete real worker stage-message localization through every renderer, and executable documented Python environment; commits cecb76c..8abc4d8)
Task 6: fix round 2/5 (3 addressed, 1 open — fifth-key secret modal can close before delayed post-create refresh disables its trigger, leaving focus on BODY; commits 8abc4d8..16bfbde)
Task 6: fix round 2/5 (4 addressed, 0 open — live fallback focus after state-changing key actions, authenticated header readiness for all captures, complete emitted worker stage mapping in header/media with dynamic slots and safe fallback, and documented `requests` prerequisite plus uv runtime command; verification: 145 backend, 63 Vitest, 74 Playwright passed with 6 intentional project skips, exact 48 visual captures)
Task 6: fix round 3/5 (1 addressed, 0 open — secret acknowledgement defers fallback focus until delayed post-create key/usage refresh renders; focused Chromium and Vitest/TypeScript passed)
Final branch review: fix pass 1 (7 addressed, 0 open — empty-database migration bootstrap dependency; durable out-of-lock filesystem deletion/restart reconciliation; type-safe server email registration validation; binary MP4 OpenAPI responses for task/preset/legacy media; real-contract locale handling for retention/disclaimer/warnings; generated workspace contract types; README upload/retention accuracy. Proven-unused phased scaffold removed; tracked SDD workspace preserved. Verification: 167 backend, 66 Vitest, TypeScript, production build, and 75 Playwright passed with 7 intentional project skips and exactly 48 visual captures.)
Final branch review: fix pass 2 (1 addressed, 0 open — every pending deletion target now fences both task and legacy retry through cleanup acknowledgement, closing the post-safety-check generation race; verification: 6 deterministic concurrency cases, 65 focused deletion/retention/retry tests, 173 full backend tests, single Alembic head, diff check; no frontend changes.)
