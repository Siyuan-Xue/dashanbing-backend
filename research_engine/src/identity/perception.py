...
from src.config import data_path, load_yaml, strict_runtime
...
def _estimate_pose_with_fallback(
    ...
    if rtmlib is None and yolo_k17 is None:
        if strict_runtime():
            raise RuntimeError("No real pose backend produced keypoints in strict runtime")
...
def _detect_persons(frame: np.ndarray, det_score_thr: float) -> list[dict]:
    ...
    if strict_runtime():
        raise RuntimeError("No real person detector is available in strict runtime")
