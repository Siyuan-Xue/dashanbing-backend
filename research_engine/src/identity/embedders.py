...
def try_load_insightface(use_gpu: bool = True):
    from src.config import strict_runtime
    ...
    try:
        ...
        root = os.environ.get("BASKETBALL_INSIGHTFACE_ROOT")
        if strict_runtime():
            if not use_gpu or providers[0] != "CUDAExecutionProvider":
                raise RuntimeError("Strict runtime requires InsightFace CUDAExecutionProvider")
            providers = ["CUDAExecutionProvider"]
            if not root:
                raise RuntimeError("BASKETBALL_INSIGHTFACE_ROOT is required")
            buffalo = Path(root) / "models" / "buffalo_l"
            expected = {
                "det_10g.onnx", "w600k_r50.onnx", "2d106det.onnx",
                "genderage.onnx", "1k3d68.onnx",
            }
            if not buffalo.is_dir() or not expected.issubset({path.name for path in buffalo.glob("*.onnx")}):
                raise FileNotFoundError("Complete local buffalo_l is required; network download is disabled")
        kwargs = {"root": root} if root else {}
        app = FaceAnalysis(name="buffalo_l", providers=providers, **kwargs)
...
def create_face_embedder(prefer_insightface: bool = True) -> FaceEmbedder:
    ...
    from src.config import strict_runtime
    if strict_runtime():
        raise RuntimeError("InsightFace buffalo_l failed to load in strict runtime")
...
def create_body_embedder() -> BodyEmbedder:
    ...
    from src.config import strict_runtime
    if strict_runtime():
        raise RuntimeError("OSNet failed to load in strict runtime")
