...
自动保留策略在 supervisor 启动时及之后每小时执行；排队和运行中的任务不会被清理：

- 未提交草稿：24 小时后标记为 `expired` 并删除任务目录；
- 注册 gallery：7 天；
- 上传、科研 raw 与科研输出：30 天；
- 已结束任务记录、结果与可视化：180 天；
- `local-assets/sample-bundle/` 是只读预置资源，永不参与清理。

用户删除和到期清理会先在同一个短数据库事务中提交逻辑变更与固定目标的清理记录，再在事务外删除文件。逻辑删除立即对 API 生效；如果文件系统清理失败或进程中断，记录会保留，并在后续清理或应用重启时安全重试。清理目标始终限制在对应的 `runtime/analyses/<uuid>/` 内。
